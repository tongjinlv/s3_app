...
extern int TurnRedOn_ForTest();
extern int TurnRedOff_ForTest();

static struct gpio_config feedback_port_cfg;
static struct input_dev *IR_feedback_dev;
static struct delayed_work alarm_work;
...
static void __worker(struct work_struct *work)
{
	...
	schedule_delayed_work(&alarm_work, msecs_to_jiffies(DELAY_TIME));
#endif
}
...
static int feedback_gpio_init()
{
	...
	type = script_get_item("IRMotionDet_para", "feedback_port", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		printk("script_get_item IRMotionDet_en_port err\n");
	...
	INIT_DELAYED_WORK(&alarm_work, __worker);
	schedule_delayed_work(&alarm_work, msecs_to_jiffies(DELAY_TIME));
